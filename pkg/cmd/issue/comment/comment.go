		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !opts.Interactive {
				if opts.Body != "" && !webMode && !editorMode {
					...
					return &cmdutil.FlagError{Err: fmt.Errorf("specify only one of --body, --web, or --editor")}
				}
			}
...
func commentRun(opts *CommentOptions) error {
	...
	if opts.InputType == inline && opts.Body == "" {
